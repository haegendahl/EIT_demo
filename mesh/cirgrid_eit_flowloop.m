function [g,gp,H,EE]=cirgrid_eit_flowloop(r,N,eI,L,el_width,bel_width);
...
% Modified by A. Seppanen 21.11.2003
...
for il = 1:L
 phi(1+(il-1)*n_pts+1:1+(il-1)*n_pts+eI(1),ii) = phi(1+(il-1)*n_pts,ii) + ...
       (2*pi/(2*pi*r(ii)))*(el_width/eI(1))*(1:eI(1))';
 phi(1+(il-1)*n_pts+eI(1)+1:1+(il-1)*n_pts+eI(1)+eI(2),ii) = ...
      phi(1+(il-1)*n_pts+eI(1),ii) + (2*pi/(2*pi*r(ii)))*(bel_width/eI(2))*(1:eI(2))';
end
