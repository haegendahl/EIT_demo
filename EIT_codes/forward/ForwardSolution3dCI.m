function [U,p,r]=ForwardSolution3dCI(Node,Element,T,sigma,z,MeasPattern,style,p,r);
...
% T=current patterns
% sigma=complex conductivity vectr
...
[A]=FemMatrix3dFast(Node,Element,sigma,z,C);
A=[real(A),-imag(A);imag(A),real(A)];
UU=A'\II1;%Voltages for the "measurement field" and for the current patterns.
UU=[UU,A\II];
U.MeasField=[UU(1:sN+L-1,1:size(II1,2));UU(sN+L:2*sN+L-1,1:size(II1,2))]; %The "measurement field" data % muutettu ottamaan enemmän
...
U.Current=[UU(1:sN+L-1,size(II1,2)+1:size(UU,2));UU(sN+L:2*sN+L-1,size(II1,2)+1:size(UU,2))]; % tähän myös
...
UU=R\(R'\[II1(p,:),II(p,:)]);
UU=UU(r,:);
U.MeasField=UU(1:sN+L-1,1:size(II1,2)); %The "measurement field" data
U.Electrode=MeasPattern'*C*UU(sN+1:size(A,1),size(II1,2)+1:size(UU,2));%Voltages on the electrodes
U.Current=UU(1:sN+L-1,size(II1,2)+1:size(UU,2));
end
