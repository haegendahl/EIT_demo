function [] = Create3DTankMeshNobarsDenseElectrodes(geofile,MaxElemSizeBoundEl)
% 
...
elec_s = 2.5;  % electrode width
elec_h = 2.5;  % electrode height
R = 25.5/2;    % radius of the tank
elec_v = 2.5;  % vertical space between electodes
taso_l = 4;  % number of electrode layers
elec_l = 16; % number of electrodes on each layer
tila_a = .9;  % space between the bottom of the tank and the bottom of the lowest electrode row
tila_y = 2;  % space between the top of the highest eletrode row and the water surface
radius = R;
...
% If numbering starts from top row
neworder = [48*8+1:64*8, 32*8+1:48*8, 16*8+1:32*8, 1:16*8];
pisteet = pisteet(:,neworder);
